	...
	var facultySubjColl = db.collection("FacultyAllocation");
			...
			facultySubjColl.find().toArray( function(error , result){
				...
				while(count !=result.length ){
					...
					getStudents(mainArr ,newObject , finalObj );
		...
		function getStudents(mainArr ,newObject , finalObj ){
		
			console.log("In get students method....mainarr = "+mainArr + " codes are "+JSON.stringify(newObject));
			SubjectsColl.aggregate([{$match:{ "_id" : { $in : mainArr } }  },{ $unwind: "$students"  },{ $lookup : { from: 							"basicUserDetails", localField: 							"students",foreignField:"_id", as:"matched"  }}   ]).toArray( function(error , result){
